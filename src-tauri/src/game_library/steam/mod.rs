...
impl TryFrom<String> for SteamAppConfigurationFile {
    // type Error = keyvalues_serde::error::Error;
    type Error = ();
...
impl Steam {
    pub fn new(install_path: &Path) -> Steam {
        ...
        let libraryfolders_vdf_content = match read_to_string(steamapps.join(LIBRARYFOLDERS_VDF_FILENAME).as_path()) {
            Ok(content) => content,
...
impl SteamLibrary for SteamLibraryfolder {
    ...
    fn get_game_acf(&self, appid: &str) -> Result<SteamAppConfigurationFile, ()> {
        ...
        match content {
            Ok(c) => match SteamAppConfigurationFile::try_from(c) {
                Ok(x) => Ok(x),
                Err(_) => Err(()),
            },
            Err(e) => Err(()),
    ...
    fn get_game_acf_list(&self) -> Result<BTreeMap<String, SteamAppConfigurationFile>, ()> {
        ...
        let result = apps.into_iter().fold((vec![], vec![]), |acc, (k, _)| {
            let mut acc = acc;
            let content = read_to_string(
                self.steamapps()
                    .join(format!("appmanifest_{}.acf", k))
                    .as_path(),
            );
            match content {
                Ok(c) => match SteamAppConfigurationFile::try_from(c) {
                    Ok(x) => acc.0.push((k, x)),
                    Err(e) => acc.1.push((k, e)),
                },
                Err(e) => acc.1.push((k, ())),
            }
            acc
...
impl TryFrom<String> for RemoteCache {
    ...
    fn try_from(content: String) -> Result<RemoteCache, Self::Error> {
        ...
        let remote_cache_item_map = object
            .into_vdfs()
            .into_iter()
